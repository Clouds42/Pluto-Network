% Modulate input bits using BPSK/QPSK/16QAM/64QAM
...
elseif ~isempty(findstr(modulation, 'QPSK'))
   ...
   table = exp(1i * [-3/4 * pi 3/4 * pi 1/4 * pi -1/4 * pi]); % generates QPSK symbols
...
elseif ~isempty(findstr(modulation, '16QAM'))
   ...
   for k = -3:2:3
      for l = -3:2:3
         table(m) = (k + 1i * l) / sqrt(10); % power normalization
         m = m + 1;
      end
   end
...
elseif ~isempty(findstr(modulation, '64QAM'))
   ...
   for k = -7:2:7
      for l = -7:2:7
         table(m) = (k + 1i * l) / sqrt(42); % power normalization
         m = m + 1;
      end
   end
